...
if TYPE_CHECKING:
    from loadshift.optimizer.machine_learning_forecaster import MLForecaster
...
def get_logger(
    fun_name: str,
    optimizer_config: dict[str, pathlib.Path],
    ...
    :type fun_name: str
    :param optimizer_config: Dictionary containing the needed optimizer paths
    ...
    if save_to_file:
        if os.path.isdir(optimizer_config["data_path"]):
            ch = logging.FileHandler(optimizer_config["data_path"] / "logger_optimizer.log")
        else:
            raise Exception("Unable to access data_path: " + optimizer_config["data_path"])
    else:
...
def calculate_surface_solar_gain(
    ...
    Reuses the existing GHI forecast that LoadShift already fetches for PV. No
...
async def treat_runtimeparams(
    ...
    default_passed_dict = {
        ...
        "custom_optim_status_id": {
            ...
            "device_class": "",
            "unit_of_measurement": "",
            "friendly_name": "LoadShift optimization status",
        },
        "custom_unit_load_cost_id": {
    ...
    if runtimeparams is not None:
        ...
        if optimizer_config["associations_path"].exists():
            async with aiofiles.open(optimizer_config["associations_path"]) as data:
            ...
            logger.warning(
                ...
                + str(optimizer_config["associations_path"])
            )
...
def get_injection_dict(df: pd.DataFrame, plot_size: int | None = 1366) -> dict:
    ...
    injection_dict = {}
    injection_dict["title"] = "<h2>LoadShift optimization results</h2>"
...
async def build_config(
    ...
    :param legacy_config_path: path to legacy config file (config_optimizer.yaml)
    ...
    # Check to see if legacy config_optimizer.yaml was provided (default /app/config_optimizer.yaml)
    ...
    if legacy_config_path and pathlib.Path(legacy_config_path).is_file():
        async with aiofiles.open(legacy_config_path) as data:
            ...
            legacy_config_parameters = await build_legacy_config_params(
                optimizer_config, legacy_config, logger
            )
            if type(legacy_config_parameters) is not bool:
                logger.info(
                    "Obtaining parameters from config_optimizer.yaml: (will overwrite config parameters)"
                )
...
async def build_legacy_config_params(
    ...
    """
    Build a config dictionary with legacy config_optimizer.yaml file.
...
async def build_secrets(
    ...
    """
    Retrieve and build parameters from secrets locations (ENV, ARG, Secrets file (secrets_optimizer.yaml/options.json) and/or Home Assistant (via API))
    ...
    :type argument: dict
    :param options_path: path to the options file (options.json) (usually provided by LoadShift-Add-on)
    :type options_path: str
    :param secrets_path: path to secrets file (secrets_optimizer.yaml)
    :type secrets_path: str
    :param no_response: bypass get request to Home Assistant (json response errors)
    :type no_response: bool
    :return: Updated optimizer_config, the built secrets dictionary
    ...
    params_secrets["hass_url"] = os.getenv("LOADSHIFT_URL", params_secrets["hass_url"])
    ...
    # Obtain secrets from options.json (Generated from LoadShift-Add-on, Home Assistant addon Configuration page) or Home Assistant API (from local Supervisor API)?
    ...
    if options_path and pathlib.Path(options_path).is_file():
        async with aiofiles.open(options_path) as data:
            ...
            # If data path specified by options.json, overwrite optimizer_config['data_path']
            ...
            if (
                ...
                # Try to create directory if it doesn't exist. if successful set data_path in optimizer_config
                try:
                    data_path = pathlib.Path(data_path_value)
                    # Use parents=True to create nested directories
                    data_path.mkdir(parents=True, exist_ok=True)
                    optimizer_config["data_path"] = data_path
            ...
            # If config path specified by options.json, overwrite optimizer_config['config_path']
            ...
            if (
                ...
                try:
                    ...
                    if config_path.exists():
                        # File exists - use it
                        optimizer_config["config_path"] = config_path
                ...
                if default_config_path.is_file():
                    ...
                    optimizer_config["config_path"] = default_config_path
                    ...
                    if default_config_dir_exists:
                        try:
                            ...
                            optimizer_config["config_path"] = legacy_config_path
                    else:
                        logger.warning(
    ...
    # Obtain secrets from secrets_optimizer.yaml? (default /app/secrets_optimizer.yaml)
    ...
    return optimizer_config, params_secrets
...
async def build_params(
    ...
    Appends configuration catagories used by optimizer to the parameters. (with use of the associations file as a reference)

    :param optimizer_config: Dictionary containing the needed optimizer paths
    :type optimizer_config: dict[str, pathlib.Path]
...
# src/loadshift/optimizer/data/associations.csv column 2. The association loop accepts
...
def log_runtime_banner(logger, optim_conf: dict | None = None):
    """Log a single INFO line with LoadShift/Python/CVXPY/platform info for bug-report reproducibility.

    When ``optim_conf`` is provided, the configured solver (or the LoadShift
    ...
    try:
        ...
        logger.info(
            f"LoadShift {_ver} | Python {_plat.python_version()} | "
        ...
        try:
            ...
            _ver = _pkg_version("loadshift")
            logger.info(f"LoadShift {_ver} (runtime info unavailable: {err})")
        except Exception:
            logger.info("LoadShift (runtime info unavailable)")
